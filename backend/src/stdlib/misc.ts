...
import { Context, isCapturedCont, Value } from '../types'
...
// dummy function. TODO: see if this function can handle the call_cc
export function call_cc(k: Value) {
  return k
}

export function reset(k: Value) {
  return k
}

export function shift(k: Value) {
  return k
}

export function is_cont(k: Value) {
  return isCapturedCont(k)
}
